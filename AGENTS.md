...
- **API** : `GET/POST /api/products` (liste/création, scoping par `clientId` si admin), `PUT/DELETE /api/products/[id]` (mise à jour partielle OK, re-valide le nom seulement s'il est fourni), `POST /api/products/upload-image` (png/jpg/jpeg/webp/gif ≤ 2 Mo → stockage `data/images/<slug>/` ou Vercel Blob), route dev `GET /api/catalog-images/[slug]/[filename]` (404 si VERCEL).
