...
export default function ClientRAGStatsPage() {
  ...
  if (!stats || stats.total === 0) {
    return (
      <div>
        <h1 className="text-2xl font-bold mb-1">Statistiques RAG</h1>
        <p className="text-gray-500 mb-6">Suivez l&apos;utilisation de votre base de connaissances.</p>
                    ...
                    <div key={day} className="flex-1 flex flex-col-reverse" style={{ height: "100%" }}>
                      <div className="flex gap-px rounded-t" style={{ height: `${h}%` }}>
                        {Object.entries(dayData).sort((a, b) => b[1] - a[1]).map(([src, count]) => {
                          const c = getBarColor(src);
                          return <div key={src} className={`${c.bg} flex-1 min-w-[2px]`} title={`${c.label}: ${count}`} />;
                        })}
                      </div>
                    </div>
                  );
