...
C+="@keyframes na-pulse{0%,100%{box-shadow:0 0 0 0 "+e.aiColor+"B3}50%{box-shadow:0 0 0 5px "+e.aiColor+"00}}";
...
C+=".nb.ai-mode{box-shadow:0 0 0 3px "+e.aiColor+",0 0 20px "+e.aiColor+"80,0 8px 32px "+e.primaryColor+"55}";
...
/* card */
C+=".nc{position:fixed;bottom:"+(e.marginBottom+76)+"px;"+op+":auto;"+e.position+":"+e.marginRight+"px;width:"+e.widgetWidth+"px;max-width:calc(100vw - 32px);height:"+e.widgetHeight+"px;max-height:calc(100vh - 120px);background:#fff;border-radius:24px;box-shadow:0 16px 48px rgba(0,0,0,.18);z-index:999998;display:none;flex-direction:column;overflow:hidden;animation:nf .32s cubic-bezier(.4,0,.2,1);transform-origin:bottom "+e.position+"}";
C+=".nc.o{display:flex}";
...
/* maximized */
C+=".nc.max{bottom:16px;"+op+":auto;"+e.position+":16px;width:"+e.widgetMaxWidth+"px;max-width:calc(100vw - 32px);height:"+e.widgetMaxHeight+"px;max-height:calc(100vh - 40px);border-radius:24px}";
...
C+=".na.ai-mode{background:linear-gradient(135deg,"+e.aiColor+","+e.aiColor+"dd)}";
C+=".na.ai-mode::after{background:"+e.aiColor+";border-color:"+e.aiColor+";box-shadow:0 0 4px "+e.aiColor+"80;animation:na-pulse 2s infinite}";
...
C+=".na-ai.on{background:"+e.aiColor+"66;box-shadow:0 0 12px "+e.aiColor+"4D}";
C+=".na-ai:hover{background:rgba(255,255,255,.25);transform:scale(1.06)}";
C+=".na-ai.on:hover{background:"+e.aiColor+"80}";
...
C+=".n-ind{display:none;align-items:center;gap:5px;background:linear-gradient(90deg,"+e.aiColor+","+e.aiColor+"dd);color:#fff;padding:3px 10px;border-radius:20px;font-size:10px;font-weight:700;letter-spacing:.3px;box-shadow:0 2px 8px "+e.aiColor+"40}";
/* messages */
...
C+=".nba.ai{background:linear-gradient(135deg,"+e.aiColor+","+e.aiColor+"dd);box-shadow:0 2px 8px "+e.aiColor+"33}";
...
C+=".nty-label{font-size:11.5px;color:"+e.aiColor+";font-weight:600;margin-bottom:7px}";
...
/* streaming cursor */
C+="@keyframes ncblink{0%,100%{opacity:1}50%{opacity:0}}";
C+=".ncur{color:"+e.aiColor+";font-weight:700;animation:ncblink .8s infinite ease-in-out;margin-left:1px}";
/* input */
...
C+=".ni-inner.ai-focus:focus-within{border-color:"+e.aiColor+";box-shadow:0 0 0 4px "+e.aiColor+"2E;background:#fff}";
...
C+=".ni-inner button#ns.ai-mode{background:"+e.aiColor+"}";
C+=".ni-inner button#ns.ai-mode:hover:not(:disabled){background:"+e.aiColor+"cc}";
...
C+=".nv-home .nw-actions{display:flex;gap:8px;max-width:260px;margin:12px auto 0}";
C+=".nv-home .nw-action-btn{flex:1;display:flex;align-items:center;justify-content:center;gap:6px;padding:10px 12px;border-radius:12px;font-size:13px;font-weight:600;cursor:pointer;transition:all .2s;border:1.5px solid #eef2f6;background:#fff;color:#374151;line-height:1.2}";
C+=".nv-home .nw-action-btn svg{width:16px;height:16px}";
C+=".nv-home .nw-action-btn.msg{background:"+e.primaryColor+";color:#fff;border-color:"+e.primaryColor+"}";
C+=".nv-home .nw-action-btn.msg:hover{opacity:.9}";
C+=".nv-home .nw-action-btn.help{background:"+e.primaryColor+"0a;color:"+e.primaryColor+";border-color:"+e.primaryColor+"22}";
C+=".nv-home .nw-action-btn.help:hover{background:"+e.primaryColor+"14}";
/* help view */
...
var ICONS={
  ...
  brain:'<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M9.5 2A2.5 2.5 0 0 1 12 4.5V12"/><path d="M9.5 2A2.5 2.5 0 0 0 7 4.5V12"/><path d="M12 12V4.5A2.5 2.5 0 0 1 14.5 2"/><path d="M14.5 2A2.5 2.5 0 0 1 17 4.5V12"/><path d="M17 12V4.5A2.5 2.5 0 0 0 14.5 2"/><path d="M9.5 22A2.5 2.5 0 0 0 12 19.5V12"/><path d="M9.5 22A2.5 2.5 0 0 1 7 19.5V12"/><path d="M12 12v7.5A2.5 2.5 0 0 0 14.5 22"/><path d="M14.5 22A2.5 2.5 0 0 0 17 19.5V12"/></svg>',
    ...
    if(source==="kb"&&score!=null){
      ...
      if(documents&&documents.length>0){
        ...
        for(var di=0;di<documents.length;di++){
          var d=documents[di];
          docLinks+='<a href="'+escAttr(d.source_url||"#")+'" target="_blank" class="nsrc" style="display:inline-flex;align-items:center;gap:3px;color:'+e.aiColor+';font-weight:600;text-decoration:none;background:#f5f3ff;padding:2px 8px;border-radius:6px;font-size:11px;margin-right:4px">\\ud83d\\udcc4 v'+d.version+'</a>';
        }
        docLinks+='</div>';
      }
...
function createStreamingBubble(){
  var box=document.getElementById("nm"),row=document.createElement("div");
  var time=formatTime();
  row.className="nmsg b";row.id="nst-"+Date.now();
  var aiCls=(aiMode)?" ai":"";
  ...
  if(messageId){
    setTimeout(function(){
      ...
      if(fbel){
        ...
        for(var si=0;si<stars.length;si++){
          (function(s){
            s.onclick=function(){if(feedState[messageId]) return;var r=parseInt(s.dataset.r);for(var x=0;x<stars.length;x++){if(parseInt(stars[x].dataset.r)<=r) stars[x].classList.add("active");else stars[x].classList.remove("active")}submitFeedback(messageId,r,s.dataset.q,s.dataset.resp,s.dataset.src,parseInt(s.dataset.scr),s.dataset.prov)};
            s.onmouseenter=function(){if(feedState[messageId]) return;var r=parseInt(s.dataset.r);for(var x=0;x<stars.length;x++){stars[x].style.color=parseInt(stars[x].dataset.r)<=r?"#f59e0b":"#d1d5db"}};
            s.onmouseleave=function(){if(feedState[messageId]) return;for(var x=0;x<stars.length;x++){stars[x].style.color=""}};
          })(stars[si]);
        }
      }
    },0);
  ...
  fetch(e.chatUrl,{method:"POST",headers:{"Content-Type":"application/json"},body:bodyPayload,signal:AbortSignal.timeout(60000)}).then(function(res){
    ...
    function processBuffer(){
      ...
      for(var li=0;li<lines.length;li++){
        var line=lines[li];
        if(line.indexOf("event: ")===0){evType=line.slice(7).trim();continue}
    ...
    function readStream(){
      reader.read().then(function(result){
        if(result.done){processBuffer();setLoading(false);return}
