...
import { Database, Search, ChevronLeft, ChevronRight, Eye, TestTube2, Upload, FileText, Loader2 } from "lucide-react";
...
export default function VectorStorePage() {
  ...
  const [perClient, setPerClient] = useState<any[]>([]);
  ...
  async function handleImport(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      if (res.ok) {
        setImportResult(`✓ ${file.name} importé — ${data.chunksCount ?? "?"} chunks`);
        loadData(1, search);
      } else {
  ...
  async function handleTest() {
    ...
    try {
      const res = await fetch("/api/vector-store", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token()}` },
        body: JSON.stringify({ question: testQuery, topN: 5 }),
      });
      const data = await res.json();
      if (data.error) { setTesting(false); return; }
  ...
  return (
    <div>
      ...
      <p className="text-gray-500 mb-6">Visualisez et testez la base pgvector.</p>
        ...
        <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
          ...
          <div className="flex items-center gap-3">
            <label className="cursor-pointer">
              ...
              <input type="file" accept=".txt,.json" className="hidden" onChange={handleImport} disabled={importing} />
            </label>
      ...
      {tab === "docs" && (
        <div>
          {stats && (
            <div className="grid grid-cols-3 gap-4 mb-6">
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <p className="text-xs text-gray-400 font-medium">Chunks totaux</p>
                <p className="text-2xl font-bold text-emerald-600">{stats.total_chunks}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <p className="text-xs text-gray-400 font-medium">Clients</p>
                <p className="text-2xl font-bold text-emerald-600">{stats.total_clients}</p>
              </div>
              <div className="bg-white rounded-xl p-4 shadow-sm border border-gray-100">
                <p className="text-xs text-gray-400 font-medium">Documents indexés</p>
                <p className="text-2xl font-bold text-emerald-600">{stats.total_docs}</p>
              </div>
            </div>
          )}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <h2 className="font-semibold text-sm mb-3">Par client</h2>
              {perClient.length === 0 ? <p className="text-gray-400 text-sm">Aucune donnée</p> : (
                  ...
                  {perClient.map((c: any) => (
                    <div key={c.clientId} className="flex items-center justify-between text-sm">
                      ...
                      <div className="flex gap-3">
                        <span className="text-emerald-600 font-medium">{c.chunks} chunks</span>
                        <span className="text-gray-400">{c.docs} docs</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="bg-white rounded-xl p-5 shadow-sm border border-gray-100">
              <h2 className="font-semibold text-sm mb-3">Top documents</h2>
              {perDoc.length === 0 ? <p className="text-gray-400 text-sm">Aucune donnée</p> : (
                <div className="space-y-2 max-h-48 overflow-auto">
                  {perDoc.map((d: any) => (
                    <div key={d.docId} className="flex items-center justify-between text-sm">
                      <div className="min-w-0 flex-1">
                        <span className="text-gray-700 truncate block">{d.source}</span>
                        <span className="text-xs text-gray-400">{d.client_name}</span>
                      </div>
                      <span className="text-emerald-600 font-medium shrink-0 ml-3">{d.chunks}</span>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div key={c.id} className="border border-gray-100 rounded-lg p-3 hover:bg-gray-50 transition-colors">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 shrink-0">
                      <span className="text-xs text-gray-400">{c.client_name}</span>
                      <button onClick={() => setViewChunk(c)} className="text-gray-400 hover:text-emerald-600 p-1" title="Voir">
                        <Eye size={13} />
                      </button>
                    </div>
                  </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setViewChunk(null)}>
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[80vh] flex flex-col m-4" onClick={e => e.stopPropagation()}>
            <div className="px-6 py-4 border-b border-gray-100">
              ...
              <p className="text-xs text-gray-400 mt-0.5">{viewChunk.client_name} / {viewChunk.chunkId}</p>
            </div>
